...
No UI dependency — used by the MCP server.
"""
...
class MydyClient:
    ...
    Auth modes (controlled by MYDY_AUTH_MODE env var):
      "scrape" (default) — form-based login, keeps a cookie session
      "token"            — extracts MoodleSession + sesskey + security keys after
                           login; enables the token-based API calls
    """

    def __init__(self):
        self.session = requests.Session()
        self.logged_in = False
        # token-mode extras (populated when MYDY_AUTH_MODE=token)
        self.moodle_session: str | None = None
        self.sesskey: str | None = None
        self.user_id: str | None = None
        self.web_key: str | None = None
        self.features_key: str | None = None
        self.my_key: str | None = None
    ...
    @staticmethod
    def _auth_mode() -> str:
        return os.getenv("MYDY_AUTH_MODE", "scrape").strip().lower()

    def _token_headers(self) -> dict:
        """Headers for token-mode requests (requires prior token login)."""
        return {"Cookie": f"MoodleSession={self.moodle_session}"}
    ...
    def login(self, username: str = "", password: str = "") -> dict:
        ...
        try:
            ...
            if has_success or ("rait" in resp.url and "login" not in resp.url):
                ...
                result: dict = {"success": True, "message": f"Logged in as {masked}", "masked_user": masked}

                if self._auth_mode() == "token":
                    token_result = self._extract_tokens(resp.text)
    ...
    def _extract_tokens(self, dashboard_html: str) -> dict:
        """Extract sesskey, user_id, and security keys from post-login HTML.

        Populates self.moodle_session / sesskey / user_id / *_key.
        Returns a dict of the extracted values for caller inspection.
        """
        self.moodle_session = self.session.cookies.get("MoodleSession")

        sesskey_match = re.search(r'sesskey["\'=:\s>]+([a-zA-Z0-9]{8,})', dashboard_html)
        self.sesskey = sesskey_match.group(1) if sesskey_match else None

        uid_match = re.search(r"/user/profile\.php\?id=(\d+)", dashboard_html)
        self.user_id = uid_match.group(1) if uid_match else None

        if self.sesskey and self.user_id:
            try:
                keys_resp = self.session.get(
                    f"{RAIT_URL}/user/managetoken.php?sesskey={self.sesskey}", timeout=10
                )
                soup = BeautifulSoup(keys_resp.text, "html.parser")
                cells = soup.select("table.generaltable tbody tr td.cell.c0")
                keys = [
                    c.get_text(strip=True) for c in cells
                    if re.fullmatch(r"[a-fA-F0-9]{32}", c.get_text(strip=True))
                ]
                while len(keys) < 3:
                    keys.append(None)
                self.web_key, self.features_key, self.my_key = keys[:3]
            except requests.RequestException:
                pass  # keys are optional; token mode still works without them
    ...
    def get_attendance(self) -> dict | str:
        ...
        # Compute overall aggregate while we have the data
        total_classes = sum(s["total_classes"] for s in subjects if isinstance(s["total_classes"], int))
        total_present = sum(s["present"] for s in subjects if isinstance(s["present"], int))
        overall_pct = round(total_present / total_classes * 100, 2) if total_classes > 0 else 0.0

        # Extract per-subject altid (used for drill-down)
        altids: dict[str, int] = {}
        if table:
            for row in table.find_all("tr"):
                cells = row.find_all("td")
                ...
                for cell in cells[1:4]:
                    p = cell.find("p")
                    if p and p.has_attr("attenid"):
                        try:
                            altids[subject_name] = int(p["attenid"])
                        except ValueError:
                            pass
                        break

        for s in subjects:
            s["altid"] = altids.get(s["subject"])
    ...
    def get_course_attendance(self, altid: int) -> list[dict] | str:
        """Per-class attendance drill-down for a single subject.

        altid comes from get_attendance() subjects[n]['altid'].
        Returns list of {class_no, subject, date, time, status} dicts.
        """
        if not self.logged_in:
            return "Not logged in."
        self._rate_limit("dashboard")
        try:
            resp = self.session.get(
                f"{RAIT_URL}/local/attendance/studentreport.php?id={altid}"
            )
            if resp.status_code != 200:
                return f"Attendance report returned status {resp.status_code}"
        except requests.RequestException as e:
            return f"Network error: {e}"
        ...
        for row in soup.select("tbody > tr"):
            ...
            status = span.get_text(strip=True) if span else cells[4].get_text(strip=True)
            records.append({
                "class_no": cells[0].get_text(strip=True),
    ...
    def get_semesters(self) -> list[dict] | str:
        """Return courses grouped by semester label (e.g. 'Semester V').

        Parses the navigation tree on the dashboard — more structured than
        list_courses() which returns a flat sorted list.
        Returns [{"semester": "Semester V", "subjects": [{"id": "...", "name": "..."}]}, ...]
        """
        if not self.logged_in:
            return "Not logged in."
        try:
        ...
        for li in soup.find_all("li", class_="type_course"):
            ...
            if not span:
                continue
            sem_name = span.get_text(strip=True)
            if not re.fullmatch(r"Semester\s+[IVX0-9]+", sem_name, re.IGNORECASE):
                continue

            p_tag = span.find_parent("p")
            ul_tag = (p_tag.find_next_sibling("ul") if p_tag else None) or li.find("ul")
            if not ul_tag:
                continue

            subjects: list[dict] = []
            for subject_li in ul_tag.find_all("li", recursive=False):
                a = subject_li.find("a", href=True)
                if not a:
                    continue
                from urllib.parse import urlparse as _urlparse
                parsed = _urlparse(a["href"])
                qs = parse_qs(parsed.query)
                if "/course/view.php" in parsed.path and "id" in qs:
                    try:
                        subjects.append({"id": str(int(qs["id"][0])), "name": a.get_text(strip=True)})
                    except (KeyError, ValueError):
                        continue

            if subjects:
                semesters.append({"semester": sem_name, "subjects": subjects})

        return semesters if semesters else "No semester data found in navigation tree."
    ...
    def submit_assignment(self, assign_url: str, file_path: str, force: bool = False) -> dict:
        ...
        if not ctx_id_match:
            # fallback: look for contextid in any script
